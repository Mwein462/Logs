import React from 'react';
import './Support.css';
const Support = () => {
  return (
    <div>
      <h2>Support Team</h2>
      <p>Text on Telegram for support</p>
      <p>Check our Telegram for news and updates</p>
      <p>I WANT TO LEARN CARDING AND WIRE FRAUD</p>
      <p>Get to learn how to spam and cashout cards and bank logs. Everything is hands-on so get ready to get your hands dirty.</p>
      <p>Requirements to enroll:</p>
      <ol>
        <li>A computer/laptop with any OS</li>
        <li>Basic computer knowledge</li>
        <li>Have Zoom or Google Meet installed</li>
        <li>A will to learn</li>
        <li>One-time payment of $200.00</li>
      </ol>
      <p>What you will get:</p>
      <ol>
        <li>In-depth learning of wire fraud, CC, and bank logs</li>
        <li>Access to us 24hrs if you have any questions or need support</li>
        <li>Access to premium private Telegram group</li>
        <li>Updated fraud bible every month</li>
        <li>Many more benefits</li>
      </ol>
      <p>TEXT AT (link) to enroll.</p>
    </div>
  );
};

export default Support;
